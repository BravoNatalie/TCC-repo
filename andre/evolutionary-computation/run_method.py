...
if __name__ == '__main__':
    ...
    today = date.today().strftime("%d-%m-%Y")
     ...
     #para executar:
     # python3 -m run_method pso 0 0 0 instances/real/instance.txt -b 100000 -r 5


    with open('instance.materials_keys-'+today+'.csv','w') as fp:
        index = list(range(0,284))
        wr = csv.writer(fp)
        wr.writerow(index)
        wr.writerow(instance.materials_keys)
    fp.close()

    with open('instance.concepts_keys-'+today+'.csv','w') as fp:
        index = list(range(0,30))
        wr = csv.writer(fp)
        wr.writerow(index)
        wr.writerow(instance.concepts_keys)
    fp.close()

    with open('instance.learners_keys-'+today+'.csv','w') as fp:
        index = list(range(0,24))
        wr = csv.writer(fp)
        wr.writerow(index)
        wr.writerow(instance.learners_keys)
    fp.close()


    selected_materials = results[0]
    
    with open('studentXselectedMaterials-'+today+'.csv','w') as fp:
        for i, seq in enumerate(selected_materials[0]):
            line = str(i)+';'+str(seq) 
            line = line.replace("[", "")
            line = line.replace("]", "")
            line = line.replace("\n", "")
    ...
    np.savetxt("student_fitnessFunction-"+today+".csv", student_fitnessFunction, fmt="%s")
